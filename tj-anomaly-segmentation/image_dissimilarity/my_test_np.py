...
############################################################################################################
config['store_results'] = "/root/autodl-tmp/data_dis/preprocess/fs_lost_and_found_val/prediction_results_synthesis_ext"
config['test_dataloader']['dataset_args']['dataroot'] = "/root/autodl-tmp/data_dis/preprocess/fs_lost_and_found_val"
###########################################################################################################
...
diss_model.eval()
model_path = os.path.join(save_fdr,
                          '%s_net_%s.pth' % (epoch, exp_name))
model_weights = torch.load(model_path)
model_weights.pop("prior_encoder.hidden_layers.0.0.weight")
model_weights.pop("prior_encoder.hidden_layers.0.0.bias")
model_weights.pop("prior_encoder.hidden_layers.1.0.weight")
model_weights.pop("prior_encoder.hidden_layers.1.0.bias")
model_weights.pop("prior_encoder.hidden_layers.2.0.weight")
model_weights.pop("prior_encoder.hidden_layers.2.0.bias")
model_weights.pop("prior_encoder.hidden_layers.3.0.weight")
model_weights.pop("prior_encoder.hidden_layers.3.0.bias")
...
with torch.no_grad():
    for i, data_i in enumerate(tqdm(test_loader)):
        ...
        # if i == 40:
        #     label1 = label.permute(1,0,2,3)
        #     label2 = label1[0]
        #     unique_class = np.unique(label2)
        #     print(unique_class)
        # else:
        #     continue
        ...
        if ensemble:
            ...
            # soft_pred = outputs[:, 1, :, :] * 0.75 + distance * 0.25
        else:
            soft_pred = outputs[:, 1, :, :]
