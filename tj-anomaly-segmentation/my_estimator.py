...
from image_dissimilarity.util import metrics
...
class AnomalyDetector():
    def __init__(self, ours=True, seed=0, fishyscapes_wrapper=True):
        ...
        self.opt.snapshot = '/root/autodl-tmp/pretrain/models/image-segmentation/cityscapes_best.pth'
        self.opt.checkpoints_dir = "/root/autodl-tmp/pretrain/models"
    ...
    def estimator_worker(self, image):
        image_og_h = image.size[0]
        image_og_w = image.size[1]
    ...
    def get_segmentation(self):
        ...
        snapshot = os.path.join(self.opt.snapshot)
    ...
    def get_synthesis(self):
        ...
        self.opt.checkpoints_dir = os.path.join(self.opt.checkpoints_dir)
    ...
    def get_dissimilarity(self, ours):
        ...
        model_path = os.path.join('/root/autodl-tmp/pretrain/models/image-dissimilarity/best_net_baseline_void_prior_spadedecoder_mult_3.pth')
...
if __name__ == '__main__':
    # import bdlb
    # # path = os.path.join(os.getcwd(), os.path.dirname(__file__), 'image_dissimilarity')
    ...
    w = 2048
    i = 0
    flat_pred = np.zeros(w * h * 500, dtype='float32')
    flat_label = np.zeros(w * h * 500, dtype='float32')

    detector = AnomalyDetector(True)


    for root, dir, files in os.walk("/root/autodl-tmp/data_dis/preprocess/1/original", topdown=False):
        for name in files:
            #cityscapes
            label = Image.open(os.path.join("/root/autodl-tmp/data_dis/preprocess/1/labels_with_ROI",name.replace("_leftImg8bit","_gtFine_labelIds")))

            #fs_lf_val
            # label = Image.open(os.path.join("/root/autodl-tmp/data_dis/preprocess/fs_lost_and_found_val/labels_with_ROI",name.replace("_leftImg8bit","_labels")))
            print(np.unique(label))
            label = torch.tensor(np.array(label))
            image = Image.open(os.path.join(root, name))
            soft_pred = detector.estimator_worker(image)
            flat_pred[i * w * h:i * w * h +
                                w * h] = torch.flatten(soft_pred).detach().cpu().numpy()
            flat_label[i * w * h:i * w * h +
                                w * h] = torch.flatten(label).detach().cpu().numpy()
            i = i + 1

    # sys.path.insert(0, os.path.join(os.getcwd(), os.path.dirname(__file__), 'image_dissimilarity'))
    # from util import metrics
    results = metrics.get_metrics(flat_label, flat_pred)
    ...
    label_save_path = '/root/autodl-tmp/data_dis/preprocess/1/prediction_result_05_19_01'
    if not os.path.exists(label_save_path):
        os.makedirs(label_save_path)
    plt.savefig(os.path.join(label_save_path, 'roc_curve.png'))
