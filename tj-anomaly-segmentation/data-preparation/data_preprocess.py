...
# to do : create a dataloader of cityscape in taming or other resynthesis models
dataloader = data.create_dataloader(opt, world_size, rank)


# to do : get the resynthesis net
model = Pix2PixModel(opt)
model.eval()

# it's a function to save the image, it's very convenient
visualizer = Visualizer(opt, rank)

# create a webpage that summarizes the all results, you can use it
...
for i, data_i in enumerate(dataloader):
    if i * opt.batchSize >= opt.how_many: # how_many means how many test images to run
        break
    generated = model(data_i, mode='inference')
    # why it use several image to inference at one time
