...
args.show_dir = '/root/autodl-tmp/data_dis/preprocess/tj-sdro-train'
####################################################################################################################################
...
if not distributed:
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        ...
        # # to check whether it has something to do with the label transform
        # overlap_original_name = 'overlap_original' +  data['img_metas'][0].data[0][0]['filename'].rsplit('/')[-1]
        # colorized_original = Image.fromarray(label_out.astype(np.uint8)).convert('P')
        # colorized_original.putpalette(original_palettes)
        # overlap_original = cv2.addWeighted(np.array(img), 0.5, np.array(colorized_original.convert('RGB')), 0.5, 0)
        # cv2.imwrite(os.path.join(overlap_original_fdr, overlap_original_name), overlap_original[:, :, ::-1])
...
batch_size = 40 # dsets.batch_size
...
image_size = (512, 256)
mask_resize = albumentations.Resize(height=image_size[1], width=image_size[0],
                                   interpolation=cv2.INTER_NEAREST)
img_resize = albumentations.Resize(height=image_size[1], width=image_size[0],
...
while start_index < len(dset):
    if start_index + batch_size > len(dset):
        indices = list(range(start_index, len(dset)))
    else:
    ...
    c = model.get_input("segmentation", example).to(model.device)
    z = model.get_input("image", example).to(model.device)
    # e = model.get_input("instance", example).to(model.device)
    with torch.no_grad() :
        quant_c, c_indices = model.encode_to_c(c)
        # quant_e, e_indices = model.encode_to_e(e)

        # ce_indices = torch.cat((c_indices, e_indices), dim=1)
        #totally don't use the input z
        ...
        #use half input z
        # quant_z, z_indices = model.encode_to_z(z)
        # z_start_indices = z_indices[:, :z_indices.shape[1] // 8]
        # index_sample = model.sample(z_start_indices, c_indices,
        ...
        x_sample = model.decode_to_img(index_sample, quant_c.shape)
        cond_rec = model.cond_stage_model.decode(quant_c)
        # e_rec = model.edge_stage_model.decode(quant_e)
        ...
        # if model.edge_stage_key == "instance":
        #     # get image from segmentation mask
        #     num_classes = e_rec.shape[1]
        #
        #     e = torch.argmax(e, dim=1, keepdim=True)
        #     e = F.one_hot(e, num_classes=num_classes)
        #     e = e.squeeze(1).permute(0, 3, 1, 2).float()
        #     e = model.edge_stage_model.to_rgb(e)
        #
        #     e_rec = torch.argmax(e_rec, dim=1, keepdim=True)
        #     e_rec = F.one_hot(e_rec, num_classes=num_classes)
        #     e_rec = e_rec.squeeze(1).permute(0, 3, 1, 2).float()
        #     e_rec = model.edge_stage_model.to_rgb(e_rec)

    x_sample = torch.clamp(x_sample, -1., 1.).to('cpu')
    x_sample = (x_sample + 1.0) / 2.0
    x_sample = x_sample.transpose(1, 2).transpose(2, 3)
    ...
    # e = torch.clamp(e, -1., 1.).to('cpu')
    # e = (e + 1.0) / 2.0
    # e = e.transpose(1,2).transpose(2,3)
    # e = e.numpy()
    # e = (e * 255)
    #
    # e_rec = torch.clamp(e_rec, -1., 1.).to('cpu')
    # e_rec = (e_rec + 1.0) / 2.0
    # e_rec = e_rec.transpose(1,2).transpose(2,3)
    # e_rec = e_rec.numpy()
    # e_rec = (e_rec * 255)

    for indice in indices:
        x_output = img_resize(image=x_sample[indice - start_index])['image']
        # x_output=x_sample[indice - start_index]
        ...
        # # c_output = mask_resize(image=c[indice - start_index])['image']
        # e_output = e[indice - start_index]
        # e_tmp = Image.fromarray(np.uint8(e_output))
        # e_tmp.save(os.path.join(temp_color_fdr,"e_origc_" + dset.labels['relative_file_path_'][indice]))
        #
        # # cond_rec_output = mask_resize(image=cond_rec[indice - start_index])['image']
        # e_rec_output = e_rec[indice - start_index]
        # e_rec_tmp = Image.fromarray(np.uint8(e_rec_output))
        # e_rec_tmp.save(os.path.join(temp_color_fdr, "e_recc_" + dset.labels['relative_file_path_'][indice]))
